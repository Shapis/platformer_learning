...
public class GameSettings : MonoBehaviour, IGameSettingsEvents
{
    ...
    [SerializeField]
    private Slider m_RightHandedSlider;

    [SerializeField]
    private Slider m_JoystickSizeSlider;

    [SerializeField]
    private Button m_ResetButton;
    public EventHandler<GameSettingsInfo> OnGameSettingsChangedEvent;
    private GameSettingsInfo myGameSettingsInfoDefault = new GameSettingsInfo
    {
    ...
    private void Awake()
    {
        ...
        try
        {
            myGameSettingsInfo = SaveHandler<GameSettingsInfo>.Load(
                SaveHandler<GameSettingsInfo>.SaveFileName.gameSettingsInfo
            );
        ...
        catch (Exception e)
        {
            Debug.Log(e);
            Debug.Log(
                "No game settings info file has been found, initializing and creating one now"
            );
            LoadGameSettings(myGameSettingsInfo);
            SaveInfo();
            //OnGameSettingsChanged(this, myGameSettingsInfo);
    ...
    {
        SaveHandler<GameSettingsInfo>.Save(
            myGameSettingsInfo,
            SaveHandler<GameSettingsInfo>.SaveFileName.gameSettingsInfo
        );
    }
    ...
    private void RightHandedModeChanged(float rightHandedMode)
    {
        ...
        {
            case 0:
                myGameSettingsInfo.rightHandedMode = false;
                break;
            case 1:
                myGameSettingsInfo.rightHandedMode = true;
                break;
        }
        SaveInfo();
        OnGameSettingsChanged(this, myGameSettingsInfo);
    ...
    private void LoadGameSettings(GameSettingsInfo gameSettingsInfo)
    {
        ...
        {
            case false:
                m_RightHandedSlider.value = 0f;
                break;
            case true:
                m_RightHandedSlider.value = 1f;
                break;
        }
    }
